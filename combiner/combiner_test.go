...
func sendTransaction(hash string) *tx.Transaction {
	tag1, _ := json.Marshal(batchchunker.ChunkInformation{PreviousChunk: "", Position: 0})
	tag2, _ := json.Marshal(batchchunker.ChunkInformation{PreviousChunk: "0xa", Position: 1})
	tag3, _ := json.Marshal(batchchunker.ChunkInformation{PreviousChunk: "0xb", Position: 2, IsHead: true})
	data1 := ([]byte("hi"))
	data2 := ([]byte("hello"))
	data3 := ([]byte("there"))
	switch hash {
	case "0xa":
		return createNewTestTransaction([]byte("0xa"), fmt.Sprintf("{\"data\": \"%s\", \"position\": %d}", utils.EncodeToBase64(data1), 0), []tx.Tag{tx.Tag{Name: arweave.BatchChunkerAppName, Value: string(tag1)}})
	case "0xb":
		return createNewTestTransaction([]byte("0xb"), fmt.Sprintf("{\"data\": \"%s\", \"position\": %d}", utils.EncodeToBase64(data2), 1), []tx.Tag{tx.Tag{Name: arweave.BatchChunkerAppName, Value: string(tag2)}})
	case "0xc":
		return createNewTestTransaction([]byte("0xc"), fmt.Sprintf("{\"data\": \"%s\", \"position\": %d}", utils.EncodeToBase64(data3), 2), []tx.Tag{tx.Tag{Name: arweave.BatchChunkerAppName, Value: string(tag3)}})
	}
	return nil
}
